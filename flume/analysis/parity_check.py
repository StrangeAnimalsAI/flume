...
Usage:
    uv run python -m flume.analysis.parity_check <jsonl> \
        [--trace-id <hex32>] \
...
from flume.backfill.claude_code import _trace_id
...
def stale_ingest_diagnostics(local: dict[str, Any], lf: dict[str, Any]) -> list[str]:
    ...
    if messages:
        messages.append(
            ...
            "trace with `python -m flume.analysis.langfuse_trace_reconcile "
...
def _cli(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m flume.analysis.parity_check",
        description=__doc__,
    )
