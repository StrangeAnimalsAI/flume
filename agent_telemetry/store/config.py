...
Read from `~/.agent-telemetry/config.toml` (override path with the
AGENT_TELEMETRY_CONFIG env var). Durations are "forever" or `<N><unit>`
...
DEFAULT_CONFIG_PATH = Path.home() / ".agent-telemetry" / "config.toml"
...
def load_policy(path: Path | None = None) -> RetentionPolicy:
    resolved = path or Path(
        os.environ.get("AGENT_TELEMETRY_CONFIG", str(DEFAULT_CONFIG_PATH))
    )
