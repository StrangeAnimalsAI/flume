...
Stdlib-only. One process, read-only queries:
    ...
    agent-telemetry-serve --port 9000 --store-url sqlite:///tmp/x.sqlite3
...
from agent_telemetry.store.base import SessionStore, open_store
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"agent-telemetry UI on http://{args.host}:{args.port}")
    try:
